...
export class SalesComponent implements OnDestroy, AfterViewInit, OnInit {
  ...
  constructor(private apiService: ApiService, private toastr: ToastrService, private excelService: ExcelService, private router: Router, private firestore: AngularFirestore) {
    this.firestore.collection('accounts').snapshotChanges().subscribe((res:any)=>{
      this.opened = res.map(e => {
  ...
  checkOpened(id:number){
    return this.opened.find(x=>x.uid == id.toString());
  }
  ...
  load(){
    ...
    this.apiService.get(`bills/Sale/${this.start.format('YYYY-MM-DD')}/${this.end.format('YYYY-MM-DD')}`).subscribe((res:any)=>{
  ...
  add(){
    let opened = this.checkOpened(this.accountId);
    if(opened == null || opened['UserName'] == this.getUserName(this.userId)){
      this.router.navigate(['/invoice/' + this.accountId + '/0']);
    }else{
      alert('Bill is opened by ' + opened['UserName'] + '. Please close first.');
    }
  }
  edit(u:any){
    let opened = this.checkOpened(this.accountId);
  ...
  download(){
    const header = ['Invoice No', 'Token No', 'Date', 'Company', 'Item', 'Qty', 'Price', 'Amount', 'Profit', 'Total', 'Past Due', 'Grand Total', 'Payment', 'Total Due',  'Remarks', 'User Name'];
    let data = [];
    ...
    console.log(data);
    this.excelService.generateExcel(header, data, 'Sales');
  }
